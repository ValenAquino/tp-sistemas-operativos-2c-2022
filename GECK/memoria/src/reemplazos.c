...
espacio_memoria_t* encontrar_espacio_victima_clock(int pid) {
	...
	log_debug(logger_debug, "Se encontro espacio de memoria victima: %d para el PID: %d", espacio_encontrado->pos_memoria, pid);
...
void aumentar_puntero_clock() {
	...
	if (puntero_clock == frames_disponibles - 1) {
		puntero_clock = 0;
		log_debug(logger_debug, "Se reinicio el puntero clock. Valor actual: %d", puntero_clock);
		return;
	}
	puntero_clock++;
	log_debug(logger_debug, "Se aumento el puntero clock. Valor actual: %d", puntero_clock);
}
