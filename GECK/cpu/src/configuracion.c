...
t_configuracion_cpu* procesar_config(t_config *config) {
	...
	int retardo_instruccion = config_get_int_value(config,
			"RETARDO_INSTRUCCION");
	char *puerto_memoria = config_get_string_value(config, "PUERTO_MEMORIA");
	char *puerto_escucha_dispatch = config_get_string_value(config,
			"PUERTO_ESCUCHA_DISPATCH");
	char *puerto_escucha_interrupt = config_get_string_value(config,
	...
	datos->puerto_escucha_dispatch = malloc(
			strlen(puerto_escucha_dispatch) + 1);
	strcpy(datos->puerto_escucha_dispatch, puerto_escucha_dispatch);

	datos->puerto_escucha_interrupt = malloc(
			strlen(puerto_escucha_interrupt) + 1);
...
void test_read_config(t_configuracion_cpu *config) {
	log_debug(logger_debug, "Leyendo de config: \n"
			...
			"REEMPLAZO_TLB: %s \n"
			"IP_MEMORIA: %s \n"
			"IP_CPU: %s \n", config->entradas_tlb, config->retardo_instruccion,
			config->puerto_memoria, config->puerto_escucha_dispatch,
			config->puerto_escucha_interrupt, config->reemplazo_tlb,
